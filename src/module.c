/* src/module.c
 * redis-fractalsql v1.0 — Stochastic Fractal Search as a Redis module.
 *
 * Stable ABI target: Redis 7.2+ (includes 8.x). This is the
 * Redis-only factory build; only APIs present in Redis 7.2 are used,
 * so the compiled .so loads unchanged on every 7.2 / 8.0 / 8.1 / 8.2
 * server. The module compiles against upstream Redis's self-contained
 * redismodule.h.
 *
 * Command
 ...
 *   LuaJIT — drop the .so in /usr/lib/redis/modules and set
 *   loadmodule in redis.conf.
 */
